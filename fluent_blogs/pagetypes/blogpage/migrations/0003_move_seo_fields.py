...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        default_choices = ('en', 'en-us', appsettings.FLUENT_BLOGS_DEFAULT_LANGUAGE_CODE)

        for blogpage in orm['blogpage.BlogPage'].objects.all():
            available_languages = list(UrlNode_Translation.objects.filter(master_id=blogpage.pk).values_list('language_code', flat=True))
            ...
            HtmlPageTranslation.objects.create(
                master_id=blogpage.pk,
                language_code=lang,
                meta_keywords=blogpage.meta_keywords,
                meta_description=blogpage.meta_description,
                meta_title=blogpage.meta_title,
            )

    def backwards(self, orm):
        # Convert all fields back to the single-language table.
        for blogpage in orm['blogpage.BlogPage'].objects.all():
            translations = orm['fluent_pages.HtmlPageTranslation'].objects.filter(master_id=blogpage.pk)
            try:
            ...
            blogpage.meta_keywords = translation.meta_keywords
            blogpage.meta_description = translation.meta_description
            blogpage.meta_title = translation.meta_title
            blogpage.save()   # As intended: doesn't call UrlNode.save() but Model.save() only.
    ...
    models = {
        ...
        u'blogpage.blogpage': {
            'Meta': {'object_name': 'BlogPage', 'db_table': "'pagetype_blogpage_blogpage'"},
