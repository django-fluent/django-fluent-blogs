...
class BlogPageTests(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.user = User.objects.create_superuser(
            "fluent-blogs-admin", "admin@example.org", "admin"
        )

    def setUp(self):
        super().setUp()
        self.overrider = override_settings(
            ROOT_URLCONF="fluent_blogs.pagetypes.blogpage.tests.urls"
        )
    ...
    def test_no_blogpage(self):
        ...
        entry = Entry.objects.language("en").create(
            author=self.user, slug="foo", publication_date=date
        )
        self.assertRaises(PageTypeNotMounted, lambda: entry.default_url)
        self.assertEqual(
            entry.get_absolute_url_format(), "/.../2016/05/{slug}/"
        )  # slug preview should not crash.
    ...
    def test_blogpage_fallback_url(self):
        ...
        page = BlogPage.objects.language("en").create(
            author=self.user, status=BlogPage.PUBLISHED, slug="blogpage"
        )
        self.assertEqual(page.default_url, "/en/blogpage/")

        date = datetime(year=2016, month=5, day=1)
        entry = Entry.objects.language("en").create(
            author=self.user, slug="hello-en", publication_date=date
        )
    ...
    def test_no_blogpage_admin(self):
        ...
        """
        date = datetime(year=2016, month=5, day=1)
        entry = Entry.objects.language("en-us").create(
            author=self.user, slug="hello-fr", publication_date=date
        )
